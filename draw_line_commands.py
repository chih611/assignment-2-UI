...
class Application(tk.Frame):
    def __init__(self, window_width, window_height, master=None):
        ...
        self.drawing_button = tk.Button(
            master, text="Open Drawing Window", command=self.open_draw_window)
        self.drawing_button.place(
            x=100, y=keyboard_bottom_y, width=200, height=30)
    ...
    def select_word_candidate(self, event):
        ...
        elif btn.cget('text') == 'Save':
            tk.messagebox.showinfo(title="Message box",
                                   message="Save function executed")
        elif btn.cget('text') == 'Cut':
            tk.messagebox.showinfo(title="Message box",
                                   message="Cut function executed")
        else:
    ...
    def open_draw_window(self):
        ...
        draw_pad_canvas = tk.Canvas(
            draw_window, width=400, height=400, bg="white")
        ...
        def draw(event):
            ...
            if last_x is not None:
                draw_pad_canvas.create_line(
                    last_x, last_y, x, y, fill="black", width=2)
            last_x, last_y = x, y

            x_coordinates.append(last_x)
            y_coordinates.append(last_y)
        ...
        def stop_drawing(event):
            ...
            print('X-----------------')
            print(x_coordinates)
            print(x_coordinates[0])
            print(x_coordinates[len(x_coordinates)-1])
            print(statistics.mean(x_coordinates))

            the_first_x = x_coordinates[0]
            the_last_x = x_coordinates[len(x_coordinates)-1]
            the_mid_x = statistics.mean(x_coordinates)

            if the_first_x > the_last_x or\
                    the_first_x < the_last_x and\
                    the_first_x > the_mid_x and\
                    the_last_x < the_mid_x:
                command_highligh = ['Save', '']
            elif the_first_x > the_mid_x and the_mid_x < the_last_x:
